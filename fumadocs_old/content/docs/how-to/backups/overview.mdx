...
In addition, the QuickStack configuration itself (projects, users, app settings) can be backed up separately — see [System Backups](./system-backups).
...
1. [Configure at least one S3 target](./s3-targets)
2. [Set up volume backup schedules](./volume-backups) for each app with persistent storage
3. [Set up database backup schedules](./database-backups) for managed databases
4. [Configure system backups](./system-backups) for QuickStack configuration
