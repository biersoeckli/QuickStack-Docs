...
> Pinning builds to a dedicated node prevents build workloads from competing with production traffic on your application nodes.
...
> If no CPU and/or memory reservation is configured, Kubernetes may schedule build containers on already-overloaded nodes. QuickStack shows an alert in the UI when reservations are missing.
